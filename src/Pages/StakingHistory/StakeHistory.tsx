...
const sub = ["Subscription", "Interest"];
const tableData = [
...
const StakeHistory = () => {
  ...
  const [selectedOption2, setSelectedOption2] = useState(sub[0]);
  ...
  <ul className="absolute left-0 top-[38px] mt-1 w-full bg-white text-black dark:bg-black dark:text-white shadow-md rounded-lg hidden group-hover:block">
    {sub.map((sub) => (
      <li
        key={sub}
        className="px-4 py-2 hover:bg-gray-500 rounded-lg cursor-pointer"
        onClick={() => setSelectedOption2(sub)}
      >
        {sub}
      </li>
    ))}

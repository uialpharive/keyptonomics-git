...
<AvailableBalance/>
      ...
      <div className="flex justify-between items-center border-b pb-4">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-gray-500">KYC {profileData.phone ? "Verified" : "Unverified"}</p>
          </div>
        </div>
      ...
      <h2 className="text-lg font-semibold text-themeBlue mb-4">Login Activity</h2>
        ...
        <table className="min-w-full border-collapse">
          ...
          <tbody>
            {currentData.map((item, index) => (
              <tr
                key={index}
                className={`relative ${index % 2 === 0 ? "bg-white rounded-lg border-transparent" : "bg-transparent"}`}
              >
                <td className={`p-3 whitespace-nowrap ${index % 2 === 0 ? "rounded-l-lg" : ""}`}>{item.date}</td>
      ...
      <div className="flex items-center justify-end gap-2 mt-4 text-gray-700">
        ...
        <div>
          {currentPage * rowsPerPage - rowsPerPage + 1}-{Math.min(
            currentPage * rowsPerPage,
            loginData.length
          )} of {loginData.length}
        </div>
